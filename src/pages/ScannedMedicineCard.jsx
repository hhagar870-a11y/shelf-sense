...
function StatPill({ icon, label, value, theme }) {
  return (
    <Box sx={{
      flexBasis: "calc(50% - 6px)", flexGrow: 0, minWidth: 0,
      p: 1.5, borderRadius: "12px", overflow: "hidden",
      ...
      <Box sx={{ minWidth: 0 }}>
        ...
        <Typography sx={{ fontSize: 15, color: theme.text, fontWeight: 800, lineHeight: 1.3, wordBreak: "break-all" }}>{value}</Typography>
      </Box>
    </Box>
  );
}
...
export default function ScannedMedicineCard({ scannedCode, theme: themeProp }) {
    ...
    async function fetchMedicines() {
      try {
        const [medsSnap, trashSnap, batchesSnap] = await Promise.all([
          ...
          // نجيب سلة المهملات كمان — عشان لو الدواء انحذف، الباركود يفضل
          // شغال ويعرض آخر معلومة معروفة عنه بدل ما يطلع "not found" وكأنه
          // ما كان موجود من الأصل
  ...
  const result = useMemo(() => {
    ...
    return { med, dates, statuses, categories, otherNames, history, lastBatch, deleted };
    ...
    <Box>
      ...
      {result && !result.notFound && (
        <Box sx={{
          ...
          {result.deleted && (
            <Box sx={{
              px: 2.5, py: 1.25, bgcolor: "#F1F5F9", borderBottom: `1px solid ${theme.border}`,
              display: "flex", alignItems: "center", gap: 1,
            }}>
              <Inventory2Icon sx={{ fontSize: 17, color: theme.muted }} />
              <Typography sx={{ color: theme.muted, fontWeight: 700, fontSize: 12.5 }}>
