...
// (مثال: البوليسي يكتب "All type of insulin" بدل ما يسرد كل ماركة — فأي دواء
// اسمه العلمي أو التجاري يحتوي هذي الكلمات يُعتبر تلقائيًا داخل نفس التصنيف)
const GENERIC_CLASS_RULES = [
  ...
  { pattern: /\btpn\b|total parenteral nutrition/i, category: "High Alert", note: 'يندرج ضمن "Total Parenteral Nutrition (TPN) Preparation" — APP 18(02)' },
];
...
export default function ClassificationSearch() {
  ...
  const searchResults = useMemo(() => {
    ...
    const tokens = requiredTokens.length ? requiredTokens : rawWords.map(normalize).filter(Boolean);
    ...
    // حتى لو ما فيه أي تطابق حرفي بالقائمة، افحصي القواعد العامة على النص المكتوب نفسه
    // (يغطي حالة أنك كاتبة اسم علمي زي "Insulin Aspart" مو موجود حرفيًا كـ keyword)
    if (matched.length === 0) {
  ...
  return (
    <Container maxWidth="md" sx={{ mt: 6, mb: 6 }}>
      ...
      {/* Mode Switch: Search vs Browse the 3 files */}
      ...
      {viewMode === "search" && (
        <>
          <TextField
            fullWidth
            autoFocus
            placeholder="Type a medication name... e.g. Warfarin"
            value={query}
          ...
          {query && searchResults.length === 0 && (
            <Box sx={{ textAlign: "center", color: "#94a3b8", py: 6 }}>
              ...
              <Typography variant="body2">
                This medication is not found in any of the classification lists (not LASA, Hazardous, or High-Alert).
              </Typography>
            </Box>
          )}
          ...
          <Box sx={{ display: "flex", flexDirection: "column", gap: 1.5 }}>
            {searchResults.map((item, idx) => (
              ...
              >
                ...
                {/* نتائج القواعد العامة (زي: يحتوي كلمة Insulin/Contrast/TPN) */}
